...
$bash_script_dir = $PSScriptRoot -replace "\\","/" -replace " ", "\\ " -replace ".:(.*)", '$1'
$letter = $($PSScriptRoot -replace "(.):.*", '$1').ToLower()

$output = & bash -c "/mnt/${letter}$bash_script_dir/project_vars.sh -exp ps" | Out-String
